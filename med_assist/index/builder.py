...
from med_assist.index import DEFAULT_MODEL, INDEX_DIR, fold_diacritics, tokenize

# Re-exported for backward compat with any caller still doing
# `from med_assist.index.builder import INDEX_DIR / DEFAULT_MODEL / tokenize`.
__all__ = ["INDEX_DIR", "DEFAULT_MODEL", "fold_diacritics", "tokenize",
           "build_dense_index", "build_sparse_index", "save", "main"]


def build_dense_index(chunks: list[Chunk], model_id: str, batch_size: int = 64) -> tuple:
    # Lazy heavy imports: keeps `import med_assist.index.builder` cheap so the
    # test runner can collect tests without pulling faiss / sentence_transformers.
    import faiss
    import numpy as np
...
def save(index_dir: Path, faiss_index, chunks: list[Chunk], bm25, manifest: dict) -> None:
    import faiss
